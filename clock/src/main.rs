...
const NTP_TO_UNIX_SECONDS: i64 = 2_208_988_800; // Number of seconds between 1 Jan 1900 (NTP epoch) and 1 Jan 1970 (UNIX epoch)
...
#[derive(Debug)]
struct NTPResult {
    t1: DateTime<Utc>, // client's timestamp when sending request
    t2: DateTime<Utc>, // server's timestamp when receiving request
    t3: DateTime<Utc>, // server's timestamp when sending response
    t4: DateTime<Utc>, // client's timestamp when receiving response
}

impl NTPResult {
    fn offset(&self) -> i64 {
        // average of 2 pairs of timestamps
        let duration = (self.t2 - self.t1) + (self.t4 - self.t3);
        duration.num_milliseconds() / 2
    }

    fn delay(&self) -> i64 {
        // time spent on client side minus time spent on server side
